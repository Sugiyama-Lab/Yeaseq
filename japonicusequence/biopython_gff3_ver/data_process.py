...
class GetSeqInfo(object):
    ...
    def __init__(self, search_type, search_content, search_organism='Schizosaccharomyces japonicus'):
        ...
        self.dict_raw_gene_info['intron'] = self.find_intermittent_site()
    ...
    def find_intermittent_site(self):
    ...
    def return_data(self):
        """
        for eacha in self.dict_raw_gene_info['intron']:
            print(self.seq_chromesome[int(eacha[0]):int(eacha[1])-1])
        for eacha in self.dict_raw_gene_info['exon']:
            print(self.seq_chromesome[int(eacha[0])-1:int(eacha[1])])
        for eacha in self.dict_raw_gene_info['CDS']:
            print(self.seq_chromesome[int(eacha[0])-1:int(eacha[1])])
        """
        list_utr5 = []
        list_utr3 = []
        if len(self.dict_raw_gene_info['exon']) == 1:
            if self.dict_raw_gene_info['exon'][0][0] != self.dict_raw_gene_info['CDS'][0][0]:
                list_utr5.append((self.dict_raw_gene_info['exon'][0][0], str(int(self.dict_raw_gene_info['CDS'][0][0])-1)))
            else:
                list_utr5 = []
            ...
            for each_exon in self.dict_raw_gene_info['exon']:
                if int(each_exon[1]) > int(self.dict_raw_gene_info['CDS'][0][0]):
                    list_utr5.append((each_exon[0], str(int(self.dict_raw_gene_info['CDS'][0][0])-1)))
                    break
                else:
                    list_utr5.append((each_exon[0], str(int(each_exon[1])+1)))
        for eacha in list_utr5:
            print(self.seq_chromesome[int(eacha[0])-1:int(eacha[1])])

        if len(self.dict_raw_gene_info['exon']) == 1:
            if self.dict_raw_gene_info['exon'][0][1] != self.dict_raw_gene_info['CDS'][0][1]:
                list_utr3.append((self.dict_raw_gene_info['CDS'][0][1], str(int(self.dict_raw_gene_info['exon'][0][0])-1)))
            else:
                list_utr3 = []
        elif len(self.dict_raw_gene_info['exon']) > 1:
            for each_exon in self.dict_raw_gene_info['exon']:
                if int(each_exon[1]) > int(self.dict_raw_gene_info['CDS'][-1][1]):
                    if not list_utr3:
                        list_utr3.append((str(int(self.dict_raw_gene_info['CDS'][-1][1])+1), each_exon[1]))
                    else:
                        list_utr3.append((each_exon[0], str(int(each_exon[1])+1)))
        for eacha in list_utr3:
            print(self.seq_chromesome[int(eacha[0])-1:int(eacha[1])])

    def print_self(self):
        print(self.str_rna_direction)
        print(self.int_start_site)
        print(self.int_end_site)


a = GetSeqInfo('genename', 'SJAG_06110')
a.print_self()
